...
let
  ...
  hetznerCloudServerNetworkOptions = { config, ... }: {
    options = {
      network = mkOption {
  ...
  hetznerCloudDiskOptions = { config, ... }: {
    ...
    options = {
      ...
      volume = mkOption {
        ...
        apply = x:
          if builtins.isString x then x else "nixops-${uuid}-${x._name}";
        description = ''
  ...
  fileSystemsOptions = { config, ... }: {
    ...
    config =
      mkAssert ((config.hetznerCloud != null) || (config.device == null)) ''
...
in {
  ...
  options = {
    ...
    deployment.hetznerCloud.ipAddresses = mkOption {
      ...
      type = with types;
        listOf (either str (resource "hetznercloud-floating-ip"));
      apply = map (x:
        if builtins.isString x then x else "nixops-" + uuid + "-" + x._name);
      description = ''
    ...
    fileSystems =
      mkOption { type = with types; loaOf (submodule fileSystemsOptions); };
  ...
  config = mkIf (config.deployment.targetEnv == "hetznercloud") {
    ...
    assertions = [{
      ...
      message =
        "Hetzner Cloud Servers can only attach to up to three networks at once.";
    }];
    ...
    deployment.hetznerCloud.volumes = mkFixStrictness (map (fs: {
      inherit (fs) mountPoint;
      inherit (fs.hetznerCloud) volume size;
      fsType =
        if fs.fsType != "auto" then fs.fsType else fs.hetznerCloud.fsType;
